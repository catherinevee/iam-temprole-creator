...
class PolicyManager:
    ...
    def create_trust_policy(self, external_id: str, allowed_departments: List[str], 
                           ip_ranges: List[str], mfa_required: bool) -> str:
        """Create trust policy for temporary role."""
        trust_policy = {
            "Version": "2012-10-17",
            "Statement": [
                {
                    "Effect": "Allow",
                    "Principal": {
                        "AWS": f"arn:aws:iam::{settings.aws_account_id}:root"
                    },
    ...
    def create_permission_boundary(self, permission_tier: PermissionTier) -> str:
        """Create permission boundary for temporary role."""
        # For now, return a basic boundary that allows most actions
